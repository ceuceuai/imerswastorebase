...
export default function HomePage(){
  ...
  const homeCfg=settings.homepage_config||{}
  const showMidBanner=(homeCfg.show_banner_mid===true||homeCfg.show_banner_mid==='on')&&!!homeCfg.banner_mid_image
  const showArticles=homeCfg.show_blog!==false&&homeCfg.show_blog!=='off'
  return <div className="store-page" style={{'--blue':settings.primary_color||'#0f67ff','--purple':settings.accent_color||'#7c3aed'}}><SeoManager settings={settings}/><StoreHeader settings={settings}/>{error&&<div className="notice container-wide">{error}</div>}<main className="container-wide">
    ...
    {showMidBanner&&<section className="homepage-mid-banner" aria-label="Banner promo">{homeCfg.banner_mid_link?<a href={homeCfg.banner_mid_link} target={/^https?:\/\//i.test(homeCfg.banner_mid_link)?'_blank':undefined} rel={/^https?:\/\//i.test(homeCfg.banner_mid_link)?'noreferrer':undefined}><img src={homeCfg.banner_mid_image} alt="Banner promo toko"/></a>:<img src={homeCfg.banner_mid_image} alt="Banner promo toko"/>}</section>}
    ...
    {showArticles&&<section id="articles" className="section article-home-section"><div className="section-head"><div><h2>📰 Artikel & Inspirasi</h2><span>Tips, informasi produk, promo, dan panduan terbaru dari toko.</span></div><a href="/articles">Lihat Semua <ArrowRight size={16}/></a></div>{articlesLoading?<div className="article-loading">Memuat artikel...</div>:<div className="article-grid">{articles.map(a=><ArticleCard key={a.id} article={a}/>)}</div>}</section>}
