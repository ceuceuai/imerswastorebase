...
export default function ProductQuickView({ product, onClose }) {
  ...
  useEffect(()=>{ setActive(0); setInputs({}) },[product?.id])
  ...
  const submit = () => {
    ...
    addItem(product, 1, inputs); onClose()
  }
  return <div className="modal-backdrop" onMouseDown={e=>e.target===e.currentTarget && onClose()}><div className="quick-modal">
    ...
    <div className="quick-media">
      <div className="main-media">{currentVideo ? (embedUrl(currentVideo) ? <iframe title="Video produk" src={embedUrl(currentVideo)} allowFullScreen/> : <video src={currentVideo} controls/>) : <img src={typeof current === 'string' ? current : product.image_url} alt={product.name}/>}</div>
      <div className="media-strip">{media.map((m,i)=><button key={i} className={active===i?'active':''} onClick={()=>setActive(i)}>{typeof m==='object'?<Play/>:<img src={m} alt={i===0?'utama':'galeri'}/>}</button>)}</div>
    </div>
    <div className="quick-info"><span className="soft-pill">{product.category?.name || product.product_type}</span><h2>{product.name}</h2><div className="quick-price">{rupiah(product.price)} {product.compare_at_price > product.price && <del>{rupiah(product.compare_at_price)}</del>}</div>
